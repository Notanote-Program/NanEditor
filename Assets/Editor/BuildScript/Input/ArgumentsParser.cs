...
namespace Milthm.BuildScript.Input
{
    public class ArgumentsParser
    {
        ...
        /// <summary>
        /// Validate and normalize buildTarget
        /// </summary>
        /// <param name="options"></param>
        /// <param name="buildTarget"></param>
        /// <returns></returns>
        private static bool ValidateBuildTarget(Dictionary<string, string> options, string buildTarget)
        {
            ...
            if (Enum.IsDefined(typeof(BuildTarget), "Standalone" + buildTarget))
            {
                var candidate = "Standalone" + buildTarget;
                options["buildTarget"] = candidate;
                return true;
            }

            if (buildTarget.StartsWith("win", StringComparison.OrdinalIgnoreCase))
            {
                var candidate = "StandaloneWindows" + buildTarget[3..];
                if (Enum.IsDefined(typeof(BuildTarget), candidate))
                {
                    options["buildTarget"] = candidate;
                    return true;
                }
        ...
        public static Dictionary<string, string> GetValidatedOptions()
        {
            ...
            if (!ValidateBuildTarget(validatedOptions, buildTarget))
            {
